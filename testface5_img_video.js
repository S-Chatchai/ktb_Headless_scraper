...
async function sendEmail(post, geminiResult, imagePath, videoPaths = []) {
    ...
    const mailOptions = {
        ...
        subject: `Gemini Result is YES - FB Post ${normalizeTimestamp(post.timestamp)}`,
        ...
        for (let i = 0; i < Math.min(postLinks.length, numPostsToScrape); i++) {
            ...
            if (postUrl.includes("/reel/")) {
                ...
                try {
                    ...
                    postData = {
                        url: postUrl,
                        fullText: pyResult.caption || "",  // ใช้ caption จาก Python
                        timestamp: new Date(),
                        imageUrl: null,
...
scrapeMultipleFBPosts().then(() => {
    ...
    const foldersToClean = [
        folderPath,             // fbpost
        geminiFolder,           // fbgeminitext
        path.join(__dirname, "downloads") // downloads ของ download_reel.py
    ];
