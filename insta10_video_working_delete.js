...
async function scrapePost(browser, url) {
  ...
  const timestamp = await page.$eval("time", el => el.getAttribute("datetime"));
...
async function analyzeWithGemini(caption, mediaPath, mediaType) {
  ...
  while (retries < maxRetries) {
    try {
      ...
      const parts = [
        {
//           text: `คุณเป็นผู้ช่วยวิเคราะห์โพสต์โซเชียลมีเดีย  

// งานของคุณคือ **ตรวจสอบว่าโพสต์นี้เกี่ยวข้องกับ สินเชื่อส่วนบุคคล, สินเชื่อบ้าน, สินเชื่อรถ, วงเงินสินเชื่อ โดยตรงหรือไม่ (เน้นย้ำต้องเกี่ยวข้องโดยตรง)** **ตอบเป็นข้อความธรรมดา (plain text) เท่านั้น** ในรูปแบบ:
// - ถ้าเกี่ยวข้อง → YES: เหตุผลสั้น ๆ 1-2 ประโยค
// - ถ้าไม่เกี่ยวข้อง → NO: เหตุผลสั้น ๆ 1-2 ประโยค

// **ข้อสำคัญ:** - พิจารณาทั้ง Caption และรูปภาพ/วิดีโอ
// - อย่าใส่ JSON, code block หรือคำอธิบายอื่น
// - นับจำนวนคนที่ปรากฎในรูปภาพ/วิดีโอด้วย
...
// --- Send email ---
async function sendEmail(post, geminiResult, mediaPath) {
  ...
  const mailOptions = {
    ...
    subject: `✅ YES - Instagram Post ${post.timestamp}`,
...
async function scrapeNewestPosts(count = 5, delaySeconds = 30) {
  ...
  for (const post of candidatePosts) {
    ...
    if (geminiOutput.toUpperCase().startsWith("เกี่ยวกับสินเชื่อ")) {
      await sendEmail(post, geminiOutput, mediaPath);
    } else {
      console.log("ℹ️ Gemini result is NO/UNKNOWN → No email sent.");
    }
